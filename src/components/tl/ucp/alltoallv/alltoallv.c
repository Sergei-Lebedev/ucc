...
ucc_status_t ucc_tl_ucp_alltoallv_pairwise_compress_start(ucc_coll_task_t *task);
ucc_status_t ucc_tl_ucp_alltoallv_pairwise_compress_progress(ucc_coll_task_t *task);
...
ucc_base_coll_alg_info_t
    ucc_tl_ucp_alltoallv_algs[UCC_TL_UCP_ALLTOALLV_ALG_LAST + 1] = {
        ...
        [UCC_TL_UCP_ALLTOALLV_ALG_PAIRWISE_COMPRESS] =
            {.id   = UCC_TL_UCP_ALLTOALLV_ALG_PAIRWISE_COMPRESS,
             .name = "pairwise with compression",
             .desc = "O(N) pairwise exchange with adjustable number "
             "of outstanding sends/recvs with data compression"},
                                                         ...
                                                         ucc_coll_task_t     **task_h)
{
    ...
    task                 = ucc_tl_ucp_init_task(coll_args, team);
    *task_h              = &task->super;
    status = ucc_tl_ucp_alltoallv_pairwise_compress_init_common(task);
