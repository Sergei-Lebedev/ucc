...
static inline size_t ucc_allgather_linear_multi_block_offset(size_t total_count,
                                                             ucc_rank_t n_blocks,
                                                             ucc_rank_t block)
{
...
static inline size_t ucc_allgather_linear_multi_block_count(size_t total_count,
                                                            ucc_rank_t n_blocks,
                                                            ucc_rank_t block)
{
    size_t block_count = total_count / n_blocks;
...
ucc_status_t ucc_tl_cuda_ipc_allgather_linear_multi_progress(ucc_coll_task_t *coll_task)
{
    ucc_tl_cuda_ipc_task_t     *task     = ucc_derived_of(coll_task,
                                                          ucc_tl_cuda_ipc_task_t);
    ucc_tl_cuda_ipc_team_t *team      = TASK_TEAM(task);
    uint32_t                coll_id   = task->allgather_linear_multi.coll_id;
    ...
    uint32_t n_tasks = UCC_TL_CUDA_IPC_TEAM_LIB(team)->cfg.linear_n_tasks *
                       team->size;
    ucc_rank_t              num_done = 0;
    size_t block_offset, block_count;
    void *src, *dst;
    ucc_rank_t i, peer, t, j, start;
    ...
    if (!task->allgather_linear_multi.sync_done) {
        ...
        task->allgather_linear_multi.sync_done = 1;
        if (UCC_IS_INPLACE(coll_task->args)) {
            start = 1;
            data_buf = PTR_OFFSET(coll_task->args.dst.info.buffer,
                                  team->rank * data_size);
        } else {
            start = 0;
        ...
        for (i = 0 ; i < n_tasks; i++) {
            block_count = ucc_allgather_linear_multi_block_count(data_size,
                                                                 n_tasks, i);
            block_offset = ucc_allgather_linear_multi_block_offset(data_size,
                                                                   n_tasks, i);
            src = PTR_OFFSET(data_buf, frag_offset + block_offset);
            exec_args.task_type = UCC_MC_EE_EXECUTOR_TASK_TYPE_BROADCAST;
            exec_args.bufs[0] = src;
            exec_args.count = block_count;
            exec_args.size = team->size - start;
            for (t = start, j = 1; t < team->size; t++, j++) {
                peer = (team->rank + t) % team->size;
                if (peer == team->rank) {
                    dst = coll_task->args.dst.info.buffer;
                } else {
                    peer_info = GET_MEM_INFO(team, coll_id, peer);
                    dst = PTR_OFFSET(task->allgather_linear_multi.peer_map_addr[peer],
                                     peer_info->offset);
                }
                dst = PTR_OFFSET(dst, team->rank * data_size +
                                 frag_offset + block_offset);
                exec_args.bufs[j] = dst;
            }
            st = ucc_ee_executor_task_post(&exec_args,
                                           &task->allgather_linear_multi.exec_task[i],
                                           task->eee);
    ...
    for (i = 0; i < n_tasks; i++) {
        if (task->allgather_linear_multi.exec_task[i]) {
            st = ucc_ee_executor_task_test(task->allgather_linear_multi.exec_task[i]);
            if (st != UCC_OK) {
                task->super.super.status = UCC_INPROGRESS;
                goto exit;
            }
            task->allgather_linear_multi.exec_task[i] = NULL;
        }
    }
...
ucc_status_t ucc_tl_cuda_ipc_allgather_linear_multi_start(ucc_coll_task_t *coll_task)
{
    ucc_tl_cuda_ipc_task_t *task = ucc_derived_of(coll_task,
                                                  ucc_tl_cuda_ipc_task_t);
    ucc_tl_cuda_ipc_team_t *team = TASK_TEAM(task);
    mem_info_t             *info = GET_MEM_INFO(team, task->allgather_linear_multi.coll_id,
                                                team->rank);
    ucc_rank_t r;

    coll_task->super.status = UCC_INPROGRESS;
    task->allgather_linear_multi.sync_done = 0;
#ifdef NVTX_ENABLED
    coll_task->id = nvtxRangeStartA("ag_ipc_start");
#endif
    UCC_TL_CUDA_IPC_PROFILE_REQUEST_EVENT(coll_task, "cuda_ipc_ag_linear_start", 0);
    for (r = 0; r < team->size * N_LINEAR_TASKS; r++) {
        task->allgather_linear_multi.exec_task[r] = NULL;
    }
    __sync_synchronize();
    asm volatile("": : :"memory");
    info->seq_num[1] = task->seq_num;

    ucc_tl_cuda_ipc_allgather_linear_multi_progress(coll_task);
...
ucc_status_t ucc_tl_cuda_ipc_allgather_linear_multi_finalize(ucc_coll_task_t *coll_task)
{
    ucc_tl_cuda_ipc_task_t *task = ucc_derived_of(coll_task,
                                                  ucc_tl_cuda_ipc_task_t);
    ucc_tl_cuda_ipc_team_t *team = TASK_TEAM(task);
    ...
    if (team->size > MAX_STATIC_SIZE) {
        ucc_free(task->allgather_linear_multi.peer_map_addr);
    }
...
ucc_status_t ucc_tl_cuda_ipc_allgather_linear_multi_setup(ucc_coll_task_t *coll_task)
{
    ...
    for (i= 0 ; i < team->size; i++) {
        ...
        if (i != team->rank && pi->d_ptr) {
            ...
            task->allgather_linear_multi.peer_map_addr[i] = mapped_addr;
        }
    }
    task->allgather_linear_multi.coll_id  = coll_id;
    return UCC_OK;
}

ucc_status_t
ucc_tl_cuda_ipc_allgather_linear_multi_post(ucc_coll_task_t *coll_task)
{
    ucc_tl_cuda_ipc_task_t *task = ucc_derived_of(coll_task, ucc_tl_cuda_ipc_task_t);
    ...
    return ucc_tl_cuda_ipc_allgather_linear_multi_start(coll_task);
}

void ucc_tl_cuda_ipc_allgather_linear_multi_done(void *data, ucc_status_t status)
{
                                                         ...
                                                         ucc_coll_task_t **task_p)
{
    ...
    if (coll_args->mask & UCC_BASE_COLL_ARGS_FIELD_EEE) {
        ...
        task->super.cb2.cb = ucc_tl_cuda_ipc_allgather_linear_multi_done;
        task->super.cb2.data = (void*)task;
    }

    if (team->size <= MAX_STATIC_SIZE) {
        task->allgather_linear_multi.peer_map_addr = task->data;
    } else {
        task->allgather_linear_multi.peer_map_addr = ucc_malloc(sizeof(void*) * team->size);
        if (!task->allgather_linear_multi.peer_map_addr) {
    ...
    task->super.progress = ucc_tl_cuda_ipc_allgather_linear_multi_progress;
    task->super.post     = ucc_tl_cuda_ipc_allgather_linear_multi_post;
    task->super.finalize = ucc_tl_cuda_ipc_allgather_linear_multi_finalize;


    ucc_tl_cuda_ipc_allgather_linear_multi_setup(&task->super);
    *task_p = &task->super;
    return UCC_OK;
